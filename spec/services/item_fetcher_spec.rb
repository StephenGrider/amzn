...
describe Services::ItemFetcher do
  before do
    f = File.open(Rails.root + "spec/fixtures/amazon_response1.xml")
    @item1 = Amazon::Ecs::Response.new(f).items.first
    @fetcher = Services::ItemFetcher.new
  end
  
  it "#get_asin" do
    expect(@fetcher.get_asin(@item1)).to eq "B0088WEN1O"
  end
  
  it "#get_price" do
    expect(@fetcher.get_price(@item1)).to eq "$60.00"
  end
  
  describe "#get_action_url" do
    it "add to wishlist" do
      expect(@fetcher.get_action_url(@item1, "Add To Wishlist")).to eq(
    ...
    it "tell a friend" do
      expect(@fetcher.get_action_url(@item1, "Tell A Friend")).to eq(
  ...
  it "#get_brand" do
    expect(@fetcher.get_brand(@item1)).to eq "ASICS"
  end
  
  it "#get_url" do
    expect(@fetcher.get_url(@item1)).to eq(
  ...
  it "#get_image_url" do
    expect(@fetcher.get_image_url(@item1)).to eq(
      "http://ecx.images-amazon.com/images/I/51GMGsRrXXL.jpg"
    )
  end
  
  it "#get_title" do
    expect(@fetcher.get_title(@item1)).to eq "ASICS Women's GEL-Venture® 4"
  end
end